...
static void enableCursor(_GLFWwindow* window)
{
    ...
    boat_server_SetCursorMode(1);
}
// Process the specified BoatInput event
//
static void processEvent(BoatInputEvent *event)
{
    int keycode = 0;
    unsigned char filtered = 0;
    
    _GLFWwindow* window = boat_server_GetCurrentWindow();
    ...
    switch (event->type)
    {
        ...
        case KeyPress:
        {
            ...
            if (event->keychar != 0)
            {
                ...
                if (window->boat.lastKeyTime < event->time)
                {
                    if (keycode)
                        _glfwInputKey(window, key, keycode, GLFW_PRESS, mods);

                    window->boat.lastKeyTime = event->time;
                }
            ...
            if (!_glfw.x11.xkb.detectable)
            {
                ...
                if (XEventsQueued(_glfw.x11.display, QueuedAfterReading))
                {
                        ...
                        next.xkey.keycode == keycode)
                    {
                        ...
                        if ((next.time - event->xkey.time) < 20)
                        {
                                   ...
                                   int visualID, int depth)
{
    ...
    window->boat.handle = boat_server_GetNativeWindow();
    
    ANativeWindow_setBuffersGeometry(window->boat.handle, 0, 0, visualID);

    if (!window->boat.handle)
    {
        _glfwInputError(GLFW_PLATFORM_ERROR,
                           "Boat: Failed to create window");
        return GLFW_FALSE;
    }
    ...
    boat_server_SetCurrentEventProcessor(window, processEvent);
                              ...
                              const _GLFWfbconfig* fbconfig)
{
    ...
    {
        visualID = 0;//DefaultVisual(_glfw.x11.display, _glfw.x11.screen);
        depth = 16;//DefaultDepth(_glfw.x11.display, _glfw.x11.screen);
    }
